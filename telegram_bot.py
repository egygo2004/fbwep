...
# Appwrite Integration for Heroku Runner
...
# Appwrite Config (Heroku Runner)
APPWRITE_ENDPOINT = os.environ.get('APPWRITE_ENDPOINT', 'https://nyc.cloud.appwrite.io/v1')
APPWRITE_PROJECT_ID = os.environ.get('APPWRITE_PROJECT_ID', '694f9a5d00376355b3a9')
APPWRITE_API_KEY = os.environ.get('APPWRITE_API_KEY', '')
APPWRITE_DATABASE_ID = os.environ.get('APPWRITE_DATABASE_ID', '694f9a8e00337e542bce')
APPWRITE_CMD_COLLECTION_ID = os.environ.get('APPWRITE_CMD_COLLECTION_ID', 'command_queue')
...
def get_server_keyboard():
    ...
    # Add Heroku Runner Option
    keyboard.append([InlineKeyboardButton("🔥 Heroku Runner (سريع)", callback_data="select_heroku_runner")])
...
async def handle_server_selection(query, context, server_key):
    ...
    # --- HEROKU RUNNER LOGIC (Appwrite) ---
    if server_key == "heroku_runner":
        del context.user_data['pending_numbers']
        
        await query.edit_message_text(
            f"🔥 **Heroku Runner**\n"
            f"📊 الأرقام: {len(numbers)}\n"
            f"⚙️ جاري الإرسال لـ Appwrite...",
            parse_mode='Markdown'
        )
        ...
        try:
            ...
            databases = AppwriteDatabases(appwrite_client)
            ...
            await query.edit_message_text(
                f"✅ **تم الإرسال لـ Heroku Runner!**\n\n"
                f"📤 تم إرسال: {success_count}/{len(numbers)}\n"
                f"🖥️ السيرفر: fb-otp-runner-hema\n\n"
                f"تابع التقدم في Appwrite Dashboard أو الموقع.",
                parse_mode='Markdown',
                reply_markup=get_main_keyboard()
            )
        except Exception as e:
            await query.edit_message_text(f"❌ خطأ في الاتصال بـ Appwrite: {e}", reply_markup=get_main_keyboard())
        return
...
async def button_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("select_"):
        server_key = data.replace("select_", "")
        await handle_server_selection(query, context, server_key)
